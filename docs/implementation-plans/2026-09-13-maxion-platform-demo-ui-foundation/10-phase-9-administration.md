...
| 9.2 | RC-05 and RC-13 via ADR-3 | `src/features/platform-prototype/admin/domain.ts#AdminState`; `src/features/platform-prototype/admin/domain.spec.ts#authority` | Task 9.1 final route semantics | `pnpm test -- admin/domain.spec.ts` |
| 9.3 | RC-13 and RC-15 via ADR-4 | `src/features/platform-prototype/admin/settings/SettingsModule.tsx#SettingsModule`; `src/features/platform-prototype/admin/integrations/IntegrationsModule.tsx#IntegrationsModule` | Task 9.2 administrative contracts | `pnpm exec playwright test tests/e2e/maxion-platform-shell.spec.ts -g 'Settings|Integrations'` |
| 9.4 | RC-13 and RC-14 via ADR-5 | `src/features/platform-prototype/admin/approvals/ApprovalsModule.tsx#ApprovalsModule`; `src/features/platform-prototype/admin/commands.ts#approvalCommand` | Task 9.3 settings and integration owners | `pnpm exec playwright test tests/e2e/maxion-platform-shell.spec.ts -g approvals` |
| 9.5 | RC-13 and RC-15 via ADR-4 | `src/features/platform-prototype/admin/usage/UsageModule.tsx#UsageModule`; `src/features/platform-prototype/admin/help/HelpModule.tsx#HelpModule`; `src/features/platform-prototype/admin/selectors.ts#selectUsage` | Task 9.4 authority-safe approvals | `pnpm exec playwright test tests/e2e/maxion-platform-shell.spec.ts -g 'Usage|Help'` |
| 9.6 | RC-15 and RC-18 via ADR-4 | `src/features/platform-prototype/admin/domain.spec.ts#break-it`; `tests/fixtures/admin-rows-10000.json#rows` | Tasks 9.3 through 9.5 complete states | `pnpm test -- admin/domain.spec.ts -t break-it` |
...
| --- | --- | --- |
| Production | Create `src/features/platform-prototype/admin/{domain.ts,selectors.ts,commands.ts,admin.css}` plus folder-owned `settings/SettingsModule.tsx`, `integrations/IntegrationsModule.tsx`, `approvals/ApprovalsModule.tsx`, `usage/UsageModule.tsx`, and `help/HelpModule.tsx`; preserve `PortalChrome` geometry; delete catch-all owners/styles | Five exclusive folder-owned route/state owners; admin rows exactly 36 px desktop/44 px mobile with 16 px icons |
