...
test("keeps the canonical MAXION shell functional across core modules", async ({ page }) => {
	...
	await expect(page.getByText("15 visual architecture diagrams")).toBeVisible()
	await expect(page.getByRole("region", { name: "Work MAX handled autonomously" })).toContainText("3conflicts resolved")
	...
	await page.getByRole("navigation", { name: "Plan workspace" }).getByRole("button", { name: /Questions/ }).click()
	...
	await expect(page.getByRole("button", { name: "Questions resolved" })).toBeVisible()
	...
	await expect(page.getByRole("button", { name: "3 approvals complete" })).toBeVisible()
