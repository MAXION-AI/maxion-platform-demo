...
test("keeps the canonical MAXION shell functional across core modules", async ({ page }) => {
	...
	await expect(page.getByRole("heading", { name: "From solution intent to team-ready build contracts." })).toBeVisible()
	...
	await expect(page.getByRole("img", { name: "L2 diagram for ServiceNow to Workday financial integration" })).toBeVisible()
	...
	await expect(page.getByRole("img", { name: "L4 diagram for ServiceNow to Workday financial integration" })).toBeVisible()
