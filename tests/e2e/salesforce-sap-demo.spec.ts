...
/*
 * The Salesforce–SAP customer demo, driven the way a presenter drives it: from the demo address,
 * through a new order sync Discovery, the handoff, and the Agentix engagement to a
 ...
 */
const DEMO = "/maxion-prototype?demo=salesforce-sap"
...
async function startOrderSyncDiscovery(page: Page, beforeCreate?: () => Promise<void>) {
	await page.getByRole("region", { name: /template/i }).getByRole("button", { name: /Salesforce–SAP order sync/ }).first().click()
	await page.getByRole("button", { name: "Start autonomous Discovery" }).click()
	await expect(page.getByRole("textbox", { name: "Mission name" })).toHaveValue("Salesforce–SAP order sync: customer master and posting integrity")
	await beforeCreate?.()
...
async function finishDiscovery(page: Page) {
	...
	await expect(page.getByRole("heading", { name: "Salesforce and SAP disagree about who the customer is" })).toBeVisible({ timeout: 25_000 })
	await page.getByRole("button", { name: "Make the SAP business partner authoritative" }).first().click()
...
async function handOff(page: Page) {
	...
	await charter.getByRole("textbox", { name: "Approval reason" }).fill("Scope, owners and the five exclusions match what revenue operations and internal controls agreed.")
...
const savedDemo = (page: Page) => page.evaluate(() => ({
	discovery: JSON.parse(localStorage.getItem("maxion.prototype.discovery-records.v1::demo-salesforce-sap") ?? "[]") as { id: string; scenarioKey: string }[],
	agentix: JSON.parse(localStorage.getItem("maxion-agentix-operations-v4::demo-salesforce-sap") ?? "null"),
}))
...
async function createEngagement(page: Page) {
	await expect(page.getByRole("heading", { name: "Review Salesforce–SAP order sync" })).toBeVisible({ timeout: 10_000 })
	await expect(page.getByText(/From Discovery · Salesforce–SAP order sync: customer master and posting integrity · packet HP-/)).toBeVisible()
	...
	await questions.getByText("The synthetic 30-day order sample").click()
...
test("the order sync demo runs from a new Discovery to a verified morning in Agentix", async ({ page }) => {
	...
	const topics = ["Failure mix · 2 of 6", "Master authority · 3 of 6", "Posting integrity · 4 of 6", "Release authority · 5 of 6", "Success measure · 6 of 6"]
	for (let index = 0; index < 6; index++) {
		await openDock(page)
		await expect(page.locator(".mxd-panel")).toContainText(`Question ${index + 1} of 6`)
		await page.locator(".mxd-panel").getByRole("button", { name: /Fill answer/ }).click()
	...
	await expect(page.getByText(/Orders fail on the way into SAP because two systems each believe they own the customer/)).toBeVisible()
	...
	await page.getByRole("textbox", { name: "Handoff note (optional)" }).fill("Start with the mapping; the unmapped-SKU rule is product operations’ to decide.")
	await handoff.getByRole("button", { name: "Continue to Agentix" }).click()
	...
	await expect(page.getByRole("heading", { name: "Salesforce–SAP order sync", exact: true })).toBeVisible()
	await expect(page.getByRole("heading", { name: "Revenue reconciliation", exact: true })).toHaveCount(0)
	await demoControls(page, "Skip")
	await page.getByRole("button", { name: "Block the order and raise it to product ops" }).click()
	await expect(dockRow(page)).toContainText("8/12")
	for (let i = 0; i < 6 && !(await page.getByRole("heading", { name: /Release order sync pipeline v2 to production\?/ }).count()); i++) await demoControls(page, "Skip")
	await expect(page.getByRole("heading", { name: /Release order sync pipeline v2 to production\?/ })).toBeVisible()
	...
	await openDock(page)
	await expect(page.locator(".mxd-panel")).toContainText("Published under ITGC-SOX-4.")
	await page.keyboard.press("Escape")

	// The previews are the beat this demo builds to: they must show THIS demo's data, never another's.
	{
		await page.getByRole("tab", { name: /^Results/ }).click()
		await page.getByRole("button", { name: /Order exception cockpit/ }).first().click()
		const results = page.locator(".aop-results-preview")
		await expect(results.getByText("Orders ·", { exact: false }).first()).toBeVisible()
		await expect(results.getByText("Posted today", { exact: false }).first()).toBeVisible()
...
test("the order sync demo keeps its own storage, leaving the other demos and the everyday prototype untouched", async ({ page }) => {
	test.setTimeout(120_000)
	await page.goto(DEMO)
	await startOrderSyncDiscovery(page)

	const keys = await page.evaluate(() => Object.keys(localStorage).sort())
	// Its own suffixed copies exist; nothing of the everyday prototype or the other demo is written.
	expect(keys).toContain("maxion.prototype.discovery-records.v1::demo-salesforce-sap")
	expect(keys.some(key => key.endsWith("::demo-revenue") || key.endsWith("::demo-servicenow"))).toBe(false)
	...
	expect(saved.discovery.some(record => record.scenarioKey === "ordersync")).toBe(true)
	expect(saved.discovery.some(record => record.scenarioKey === "revenue" || record.scenarioKey === "servicenow")).toBe(false)
...
test("every new order sync Discovery starts Agentix again from zero", async ({ page }) => {
	test.setTimeout(240_000)
	...
	await startOrderSyncDiscovery(page, async () => {
		await expect(replaceNote(page)).toContainText("replaces the earlier Salesforce–SAP order sync Discovery and starts Agentix again from zero")
	...
	const saved = await savedDemo(page)
	const records = saved.discovery.filter(record => record.scenarioKey === "ordersync")
	expect(records).toHaveLength(1)
	const agentix = saved.agentix as { engagements: Record<string, { status: string; packages: string[] }>; work: { engagementId: string }[] }
	expect(agentix.engagements.orders.status).toBe("draft")
	expect(agentix.engagements.orders.packages).toEqual([])
	expect(agentix.work.filter(entry => entry.engagementId === "orders")).toEqual([])
})
