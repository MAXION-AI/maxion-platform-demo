...
PLAN_DIRECTORY = "docs/implementation-plans/2026-09-13-maxion-platform-demo-ui-foundation"
...
CANONICAL_COMMANDS = {
    ...
    "diff-check": "git diff --check {rangeBaseSha} {rangeHeadSha} --",
}
CANONICAL_EXECUTION = {
    "check-program": [["pnpm", "check:program"]],
    "build": [["pnpm", "build"]],
...
def _canonical_command(command_id: str, *, base_sha: str, run_sha: str) -> str:
    command = CANONICAL_COMMANDS[command_id]
    if command_id == "diff-check":
        return command.format(rangeBaseSha=base_sha, rangeHeadSha=run_sha)
    return command


def _canonical_execution(command_id: str, *, base_sha: str, run_sha: str) -> list[list[str]]:
    if command_id == "diff-check":
        return [["git", "diff", "--check", base_sha, run_sha, "--"]]
    return CANONICAL_EXECUTION[command_id]
...
def _validate_commands(record: dict[str, Any], *, accepted: bool) -> None:
    ...
    expected_keys = {
        ...
        "stderrSha256", "evidencePath", "evidenceSha256", "rangeBaseSha", "rangeHeadSha",
    }
    for result in commands:
        ...
        expected_command = _canonical_command(
            command_id, base_sha=record["baseSha"], run_sha=result["runSha"]
        )
        if result["command"] != expected_command:
            raise LedgerError(f"command {command_id!r} does not match its canonical invocation")
        expected_range = (
            (record["baseSha"], result["runSha"])
            if command_id == "diff-check" else (None, None)
        )
        if (result["rangeBaseSha"], result["rangeHeadSha"]) != expected_range:
            raise LedgerError(f"command {command_id!r} range revisions are invalid")
...
def _validate_post_merge_commands(record: dict[str, Any], *, required: bool) -> None:
    ...
    expected_keys = {
        ...
        "stdoutSha256", "stderrSha256", "rangeBaseSha", "rangeHeadSha",
    }
    ids: list[str] = []
    for result in results:
        ...
        if command_id not in CANONICAL_COMMANDS or result.get("command") != _canonical_command(
            str(command_id), base_sha=record["baseSha"], run_sha=record["mergeSha"]
        ):
        ...
        expected_range = (
            (record["baseSha"], record["mergeSha"])
            if command_id == "diff-check" else (None, None)
        )
        if (result.get("rangeBaseSha"), result.get("rangeHeadSha")) != expected_range:
            raise LedgerError("post-merge command range revisions are invalid")
...
def _validate_review_report_shape(
    ...
    if role == "qa" and scope_kind != "package":
        ...
        for run in runs:
            ...
            artifact_hashes.append(run["artifactSha256"])
        ...
        if len(set(artifact_hashes)) != 3:
            raise LedgerError("QA browser runs must bind three distinct committed artifact contents")
        if scope_kind == "all-surfaces":
            browsers = {str(run["browser"]).strip().casefold() for run in runs}
            if browsers != {"chromium", "webkit"}:
                raise LedgerError(
                    "all-surfaces QA browser runs must cover exactly the Chromium and WebKit projects"
                )
...
def _validate_command_report_shape(report: Any, *, record: dict[str, Any], result: dict[str, Any]) -> None:
    expected = {
        ...
        "startedAt", "finishedAt", "rangeBaseSha", "rangeHeadSha",
    }
    if not isinstance(report, dict) or set(report) != expected:
...
def _phase_scope(record: dict[str, Any], repo_root: Path = ROOT) -> tuple[str, set[str]]:
    ...
    if matching:
        ...
        if scope.get("kind") == "phase-surfaces" and not scoped:
            raise LedgerError("phase-surfaces acceptance scope must name at least one reference sheet")
        if not scoped.issubset(all_sheets):
            raise LedgerError("acceptance scope names an undeclared reference sheet")
        if scope.get("kind") not in {"phase-surfaces", "all-surfaces", "package"}:
...
def _validate_git_and_artifacts(record: dict[str, Any], repo_root: Path) -> None:
    ...
    if record["status"] == ACCEPTED_STATUS:
        ...
        for path, expected_hash in expected_candidate_artifacts.items():
        ...
        for path, expected_hash in record["referenceSheetSha256"].items():
            if _git_blob_sha256(repo_root, record["evidenceCommitSha"], path) != expected_hash:
                raise LedgerError(f"recorded completed-sheet SHA-256 does not match {path} at evidence E")
...
def _run_canonical_commands(record: dict[str, Any], repo_root: Path) -> list[dict[str, Any]]:
    ...
    for command_id in CANONICAL_COMMANDS:
        ...
        stderr = bytearray()
        exit_code = 0
        for argv in _canonical_execution(
            command_id, base_sha=str(record["baseSha"]), run_sha=str(record["mergeSha"])
        ):
            try:
                result = run_bounded(
        ...
        receipts.append({
            ...
            "rangeBaseSha": record["baseSha"] if command_id == "diff-check" else None,
            "rangeHeadSha": record["mergeSha"] if command_id == "diff-check" else None,
            "stdout": stdout_text,
            "stderr": stderr_text,
...
def _phase_handoff(repo_root: Path, evidence: str, phase: int) -> dict[str, Any]:
    names = str(
        _git(repo_root, "ls-tree", "-r", "--name-only", evidence, "--", PLAN_DIRECTORY)
    ).splitlines()
    pattern = re.compile(rf"^{re.escape(PLAN_DIRECTORY)}/{phase + 1:02d}-phase-{phase}-.+\.md$")
    paths = [path for path in names if pattern.fullmatch(path)]
    if len(paths) != 1:
        raise LedgerError("prepare requires exactly one predetermined phase document at evidence E")
    text = _git_blob(repo_root, evidence, paths[0]).decode("utf-8")
    match = re.search(
    ...
    if not match:
        raise LedgerError("prepare cannot find the structured acceptance hand-off at evidence E")
    try:
        handoff = json.loads(match.group(1))
    except json.JSONDecodeError as exc:
        raise LedgerError("prepare found an invalid structured acceptance hand-off") from exc
    if not isinstance(handoff, dict) or handoff.get("phase") != phase or handoff.get("status") != "accepted":
        raise LedgerError("prepare requires the accepted structured hand-off for this phase")
    return handoff
...
def prepare_raw_record(
    *,
    ...
    """Derive the complete append input from committed C/E/M objects without hand editing."""

    for value, field in ((base, "base"), (candidate, "candidate"), (evidence, "evidence"), (merge, "merge")):
        _validate_sha(value, field)
        _require_commit(repo_root, value, field)
    if not isinstance(pr_number, int) or pr_number <= 0:
        raise LedgerError("prepare pr-number must be a positive integer")
    handoff = _phase_handoff(repo_root, evidence, phase)
    evidence_paths = handoff.get("evidence")
    if not isinstance(evidence_paths, list) or not evidence_paths:
        raise LedgerError("prepare requires the structured hand-off to list committed evidence")
    review_reports: dict[str, dict[str, str]] = {}
    review_values: dict[str, dict[str, Any]] = {}
    for role in ("ux", "qa"):
        path = f"artifacts/ux-audits/phase-{phase}/independent-{role}.json"
        try:
            report = json.loads(_git_blob(repo_root, evidence, path))
        except json.JSONDecodeError as exc:
            raise LedgerError(f"prepare found invalid {role} review JSON") from exc
        if not isinstance(report, dict):
            raise LedgerError(f"prepare found invalid {role} review JSON")
        review_values[role] = report
        review_reports[role] = {"path": path, "sha256": _git_blob_sha256(repo_root, evidence, path)}
    builder = review_values["ux"].get("builder")
    reviewers = {role: review_values[role].get("reviewer") for role in ("ux", "qa")}
    command_results: list[dict[str, Any]] = []
    for command_id in CANONICAL_COMMANDS:
        path = f"artifacts/ux-audits/phase-{phase}/command-{command_id}.json"
        try:
            report = json.loads(_git_blob(repo_root, evidence, path))
        except json.JSONDecodeError as exc:
            raise LedgerError(f"prepare found invalid command report {command_id!r}") from exc
        if not isinstance(report, dict):
            raise LedgerError(f"prepare found invalid command report {command_id!r}")
        command_results.append({
            **{key: report.get(key) for key in (
            ...
            "evidencePath": path,
            "evidenceSha256": _git_blob_sha256(repo_root, evidence, path),
        })
    commit_time = str(_git(repo_root, "show", "-s", "--format=%cI", merge))
    try:
        timestamp = datetime.strptime(commit_time, "%Y-%m-%dT%H:%M:%S%z").astimezone(
            timezone.utc
        ).strftime("%Y-%m-%dT%H:%M:%SZ")
    except ValueError as exc:
        raise LedgerError("prepare cannot normalize the merge commit timestamp") from exc
    record: dict[str, Any] = {
        "phase": phase,
        ...
        "prHeadSha": evidence,
        "mergeSha": merge,
        "mergeCommitSubject": str(_git(repo_root, "show", "-s", "--format=%s", merge)),
        "targetRef": TARGET_REF,
        "independentAcceptanceSha": candidate,
        "evidenceCommitSha": evidence,
        "sourceTreeSha1": str(_git(repo_root, "rev-parse", f"{candidate}:src")),
        "protectedObjectSha1": {
            path: _git_object(repo_root, candidate, path) for path in PROTECTED_OBJECT_PATHS
        },
        "lockSha256": _git_blob_sha256(repo_root, candidate, "pnpm-lock.yaml"),
        "manifestSha256": _git_blob_sha256(
            repo_root, candidate, "docs/operations/figma-code-map.json"
        ),
        "referenceSheetSha256": {
            path: _git_blob_sha256(repo_root, evidence, path) for path in REFERENCE_SHEET_PATHS
        },
        "referenceSheetContractSha256": {
            path: reference_sheet_contract_sha256(_git_blob(repo_root, candidate, path))
            for path in REFERENCE_SHEET_PATHS
        },
        "builder": builder,
        "reviewers": reviewers,
        "reviewReports": review_reports,
        "evidence": evidence_paths,
        "evidenceQualification": ACCEPTED_EVIDENCE_QUALIFICATION,
        "reasonOpen": None,
        "commands": command_results,
        "timestamp": timestamp,
    }
    validate_record(
        record, external=False, repo_root=repo_root, require_post_merge=False
    )
    _validate_evidence_only(record, repo_root)
    return record


def export_tracked_snapshot(
    external_path: Path,
    output_path: Path,
    *,
    candidate_phase: int,
    ...
    """Export accepted authority records and advance exactly to the next candidate phase."""

    document = _load(external_path)
    validate_external(document, target_ref=target_ref, repo_root=repo_root)
    records = document["records"]
    if candidate_phase != len(records):
        raise LedgerError(
            f"tracked export candidatePhase must be exactly {len(records)} after the accepted prefix"
        )
    snapshot = {
        "schemaVersion": 1,
        "program": PROGRAM,
        "repository": REPOSITORY,
        "externalAuthority": str(DEFAULT_EXTERNAL_LEDGER),
        "candidatePhase": candidate_phase,
        "snapshotQualification": "lagging-export-of-external-accepted-records-only",
        "snapshotThroughPhase": len(records) - 1,
        "phases": records,
    }
    validate_tracked(snapshot, repo_root=repo_root)
...
def main(argv: list[str] | None = None) -> int:
    ...
    prepare_parser.add_argument("--phase", required=True, type=int)
    ...
    prepare_parser.add_argument("--merge", required=True)
    prepare_parser.add_argument("--pr-number", required=True, type=int)
    prepare_parser.add_argument("--output", required=True, type=Path)
    prepare_parser.add_argument("--repo", type=Path, default=ROOT)
    export_parser = subparsers.add_parser("export-tracked")
    export_parser.add_argument("--ledger", type=Path, default=DEFAULT_EXTERNAL_LEDGER)
    export_parser.add_argument("--output", type=Path, default=TRACKED_LEDGER)
    export_parser.add_argument("--candidate-phase", required=True, type=int)
    export_parser.add_argument("--target-ref", required=True)
    ...
    try:
        if args.command == "validate":
            ...
            with _locked(args.output):
                if args.output.exists():
                    raise LedgerError("prepare refuses to overwrite an existing raw record")
                _atomic_write(args.output, record)
            print(f"Program ledger: prepared raw phase {record['phase']} record at {args.output}")
        elif args.command == "export-tracked":
            snapshot = export_tracked_snapshot(
                args.ledger,
                args.output,
            ...
            print(
                f"Program ledger: exported through phase {snapshot['snapshotThroughPhase']} "
                f"for candidate phase {snapshot['candidatePhase']}"
            )
        else:
