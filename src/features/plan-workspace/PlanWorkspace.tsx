...
export function PlanWorkspace({ projects, discoveryPackage = null, onSendToExecute, onNavigate, jumpSignal = null }: PlanWorkspaceProps) {
	...
	const regenerate = (section: PlanSection) => {
		const baseVersion = artifact.currentVersion
		const meta = commandMeta("regenerate", section.id)
		const operationId = meta.idempotencyKey
		send({ type: "section/regeneration-started", projectId: artifact.projectId, sectionId: section.id, baseVersion, operationId, actorRole: role, ...meta })
		if (generationTimer.current !== null) window.clearTimeout(generationTimer.current)
		generationTimer.current = window.setTimeout(() => send({ type: "section/regeneration-proposed", projectId: artifact.projectId, sectionId: section.id, baseVersion, operationId, actorRole: role, body: `${section.body}\n\nUpdated with the current evidence boundary and accountable owner.` }), 240)
	}
	...
	const verifiedGapSource = artifact.sources.find((source) => source.verified) ?? null
...
function ArtifactEditor({ artifact, section, role, dispatch, onStartEdit, onSave, onRegenerate, commandMeta }: { artifact: PlanArtifact; section: PlanSection; role: PlanRole; dispatch: (command: PlanCommand) => void; onStartEdit: (section: PlanSection) => void; onSave: (section: PlanSection) => void; onRegenerate: (section: PlanSection) => void; commandMeta: (action: string, sectionId?: string) => { idempotencyKey: string; correlationId: string } }) {
	...
	return <article className="pnw-artifact"><header><div><small>{section.id.toUpperCase()}</small><h2>{section.title}</h2></div><span className={`is-${section.status}`}>{section.status.replace("-", " ")}</span></header>{editing ? <textarea className="pnw-editor" aria-label={`Edit ${section.title}`} value={artifact.drafts[section.id]} maxLength={20_000} autoFocus onChange={(event) => dispatch({ type: "section/draft-changed", projectId: artifact.projectId, sectionId: section.id, value: event.target.value, actorRole: role })} rows={12} /> : <p className="pnw-copy">{section.body}</p>}
		<div className="pnw-artifact-actions">{editing ? <><button type="button" onClick={() => dispatch({ type: "section/edit-cancelled", projectId: artifact.projectId, sectionId: section.id, actorRole: role })}>Cancel</button><button type="button" className="is-primary" onClick={() => onSave(section)}>Save section</button></> : <><button type="button" disabled={readOnly} onClick={() => onStartEdit(section)}>Edit section</button><button type="button" disabled={readOnly || section.generation.status === "running"} onClick={() => onRegenerate(section)}>{section.generation.status === "running" ? <><SpinnerGap className="pnw-spin" />Generating…</> : <><MagicWand />Regenerate</>}</button></>}</div>
		...
		{section.generation.status === "proposed" && section.generation.proposedBody && section.generation.operationId && section.generation.baseVersion !== null ? <section className="pnw-generation"><MagicWand /><div><strong>Proposed revision</strong><p>{section.generation.proposedBody}</p></div><button type="button" onClick={() => dispatch({ type: "section/regeneration-rejected", projectId: artifact.projectId, sectionId: section.id, baseVersion: section.generation.baseVersion as number, operationId: section.generation.operationId as string, actorRole: role })}>Reject</button><button type="button" className="is-primary" onClick={() => dispatch({ type: "section/regeneration-accepted", projectId: artifact.projectId, sectionId: section.id, baseVersion: section.generation.baseVersion as number, operationId: section.generation.operationId as string, actorRole: role, ...commandMeta("accept-regeneration", section.id) })}>Accept</button></section> : null}
