...
export function PlanLibraryModule({ projects, onOpenPlan, onNavigate }: { projects: PortalProject[]; onOpenPlan: () => void; onNavigate: Navigate }) {
	...
	return (
		<div className="mxp-portal-page mxp-plan-library">
			...
			{createOpen ? <div className="mxp-dialog-layer" onMouseDown={(event) => { if (event.currentTarget === event.target) setCreateOpen(false) }}><section role="dialog" aria-modal="true" aria-labelledby="create-plan-title" className="mxp-portal-dialog mxp-plan-create-dialog"><header><div><span className="mxp-dialog-icon"><Sparkle size={18} /></span><div><small>Autonomous plan</small><h2 id="create-plan-title">Give MAX a starting point</h2></div></div><button type="button" aria-label="Close create plan dialog" onClick={() => setCreateOpen(false)}><X size={17} /></button></header><form onSubmit={startPlan}><label>Project<select value={projectId} onChange={(event) => setProjectId(event.target.value)} required>{projects.filter((project) => project.status === "active" && project.role !== "Viewer").map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></label><fieldset className="mxp-plan-source-picker"><legend>Start with</legend>{planSources.map(({ id, label, detail, icon: Icon }) => <button key={id} type="button" aria-pressed={source === id} onClick={() => setSource(id)}><span><Icon size={16} /></span><strong>{label}</strong><small>{detail}</small>{source === id ? <CheckCircle size={15} weight="fill" /> : null}</button>)}</fieldset><label>{source === "manual" ? "What should this plan accomplish?" : "Anything MAX should know? (optional)"}<textarea autoFocus={source === "manual"} value={objective} onChange={(event) => setObjective(event.target.value)} rows={3} placeholder={source === "manual" ? "Describe the outcome and any hard constraints." : "Add a constraint, decision horizon, or outcome—MAX will infer the rest."} required={source === "manual"} /></label><div className="mxp-plan-auto-note"><MaxionSpiralMark /><div><strong>MAX takes it from here</strong><p>It will assemble evidence, decompose every flow through L2–L4, test the implementation guidance, and only interrupt for decisions it cannot safely infer.</p></div></div><p className="mxp-form-note"><ShieldCheck size={14} />Membership, evidence policy, cost limits, and approvals inherit from the project.</p><div><button type="button" onClick={() => setCreateOpen(false)}>Cancel</button><button type="submit" className="mxp-primary" disabled={!projectId || (source === "manual" && !objective.trim())}>Start autonomous plan<ArrowRight size={14} /></button></div></form></section></div> : null}
		</div>
	)
}
...
function LegacyExecuteHubModule({
	onOpenRun,
	onNavigate,
	planHandoff,
}: {
	...
	return (
		<div className="mxp-portal-page mxp-execute-hub">
			<PortalPageHeader
				eyebrow="Execute"
				title="Autonomous delivery"
				description="Delegate an outcome. MAX plans, builds, tests, repairs, and returns with evidence—or one precise question."
				actions={
					<>
						<button type="button" onClick={() => onNavigate("integrations")}><Plug size={15} />Tools</button>
						<button type="button" aria-label="Execute notifications"><BellRinging size={16} /></button>
						<button type="button" className="mxp-primary" onClick={() => { setView("engagements"); setSource("prompt"); window.setTimeout(() => composerRef.current?.focus(), 0) }}><Plus size={16} />New task</button>
					</>
				}
			/>
			{view === "engagements" ? (
				<div className="mxp-execute-home">
					<section className="mxp-execute-delegate" aria-labelledby="execute-delegate-title">
						<div className="mxp-execute-delegate-intro">
							<MaxionSpiralMark className="mxp-execute-delegate-mark" />
							<div><small>MAX Execute</small><h2 id="execute-delegate-title">What should MAX deliver?</h2><p>Describe the result, not the steps. MAX will work out the implementation path and ask only when authority is required.</p></div>
						</div>
						<form className="mxp-execute-composer" onSubmit={launchEngagement}>
							...
							{source === "prompt" ? (
								<textarea ref={composerRef} aria-label="What should Execute deliver?" value={prompt} onChange={(event) => setPrompt(event.target.value)} rows={5} placeholder="Build the approved mission-authority boundary, preserve the current API, and return only after the release gate is clean." />
							) : (
								<fieldset className="mxp-plan-picker mxp-execute-plan-picker"><legend>Choose an approved Plan</legend>{availablePlans.map((plan) => <button key={plan.id} type="button" aria-pressed={selectedPlanId === plan.id} onClick={() => setSelectedPlanId(plan.id)}><span className="mxp-plan-item-icon"><FlowArrow size={16} /></span><span><strong>{plan.name}</strong><small>{plan.project} · {plan.detail}</small></span><CheckCircle size={16} /></button>)}</fieldset>
							)}
							<footer>
								<div><span><Code size={13} />max-ai-platform</span><span><ShieldCheck size={13} />Governed worktrees</span></div>
								<button type="submit" className="mxp-primary" disabled={source === "prompt" ? !prompt.trim() : !selectedPlan}>Start engagement<ArrowRight size={15} /></button>
							</footer>
						</form>
						<p className="mxp-execute-delegate-note"><CheckCircle size={15} />MAX plans, creates isolated workspaces, implements, tests, repairs, and returns verified evidence.</p>
					</section>

					<section className="mxp-execute-recents" aria-label="Active Execute engagements">
						<header><div><h2>Continue work</h2><p>Long-running tasks keep working when you leave.</p></div><span>2 active</span></header>
						{[
							{ name: "ERP modernization delivery", status: "Running", progress: 67, detail: "Running cumulative verification", updated: "2 min", brief: "Implement the approved ERP modernization outcomes with tenant-safe authority boundaries." },
							{ name: "Customer data foundation", status: "Ready", progress: 20, detail: "Topology prepared · waiting to start", updated: "18 min", brief: "Deliver the approved customer data foundation and verify every integration boundary." },
						].map((run) => (
							<button type="button" key={run.name} onClick={() => onOpenRun({ source: "plan", title: run.name, brief: run.brief, autoStart: run.status === "Running" })}>
								<span className={`mxp-execute-run-state is-${run.status.toLowerCase()}`}><i /></span>
								<span><strong>{run.name}</strong><small>{run.detail}</small></span>
								<span className="mxp-execute-run-meta"><small>{run.progress}%</small><time>{run.updated}</time><ArrowRight size={15} /></span>
							</button>
						))}
					</section>
				...
				<section className="mxp-approval-inbox mxp-execute-approval-view">
					<header><button type="button" onClick={() => setView("engagements")}><ArrowRight size={14} />Back to Execute</button><div><h2>Execute approvals</h2><p>Only decisions that widen authority or create an external effect appear here.</p></div><StatusSummary approved={approved} /></header>
					{approved ? <div className="mxp-approval-empty"><CheckCircle size={26} /><h3>All caught up</h3><p>MAX can continue without your input.</p></div> : <article><span><ShieldCheck size={19} weight="duotone" /></span><div><small>Repository authority · ERP modernization</small><h3>Approve the generated workspace map</h3><p>Five isolated workspaces will bind to max-ai-platform with explicit branches, allowed paths, and no deployment authority.</p><dl><div><dt>Repositories</dt><dd>1</dd></div><div><dt>Workspaces</dt><dd>5</dd></div><div><dt>Deploy authority</dt><dd>None</dd></div></dl></div><div><button type="button">Inspect scope</button><button type="button" className="mxp-primary" onClick={() => setApproved(true)}>Approve binding</button></div></article>}
				</section>
			)}
		</div>
	)
