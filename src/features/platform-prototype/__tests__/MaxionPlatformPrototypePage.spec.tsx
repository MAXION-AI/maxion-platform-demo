...
describe("MaxionPlatformPrototypePage", () => {
	...
	it("creates an autonomous Plan from existing context and provides L2, L3, and L4 guidance for every flow", () => {
		...
		expect(screen.getByRole("img", { name: "L2 diagram for Mission authority and approval boundary" })).toBeInTheDocument()
		fireEvent.click(screen.getByRole("button", { name: "L3 Contracts" }))
		expect(screen.getByRole("img", { name: "L3 diagram for Mission authority and approval boundary" })).toBeInTheDocument()
		...
		expect(screen.getByRole("img", { name: "L4 diagram for ServiceNow financial-change event intake" })).toBeInTheDocument()
