...
describe("MaxionPlatformPrototypePage", () => {
	...
	it("embeds Agentix and lets Consult MAX route across platform boundaries", async () => {
		renderPrototype()
		expect(screen.getByRole("complementary", { name: "Main navigation" })).toBeInTheDocument()
		await waitFor(() => expect(portalNavigation().getByRole("button", { name: /^Agentix 2 pending$/ })).toBeInTheDocument())

		fireEvent.click(screen.getByRole("button", { name: "Consult Max" }))
		expect(await screen.findByLabelText("Consult MAX workspace")).toBeInTheDocument()
		expect(screen.getByRole("complementary", { name: "Consult sources" })).toBeInTheDocument()
		expect(screen.getAllByRole("button", { name: /Discover|Plan|Execute|Agentix/ }).length).toBeGreaterThanOrEqual(4)
		const composer = await screen.findByLabelText("Message Consult MAX")
		fireEvent.change(composer, { target: { value: "What needs Agentix approval?" } })
		fireEvent.click(screen.getByRole("button", { name: "Send to Consult MAX" }))
		expect(await screen.findByText(/\$240 invoice variance is the current bounded decision/)).toBeInTheDocument()
		fireEvent.click(screen.getByRole("button", { name: "Open in Agentix" }))
		expect(await screen.findByRole("region", { name: "Agentix module" })).toBeInTheDocument()
		expect(portalNavigation().getByRole("button", { name: /^Agentix/ })).toHaveAttribute("aria-current", "page")
	})
})
